...
class YoloTest(object):
    ...
    def evaluate(self):
        ...
        for task_id in file_info_list:
            if task_id in ['MKZ156_20201215112828','MKZ073_20201216225330','HQEV503_20201215091205']:#
                ...
                for video_id in task_dict:
                    ...
                    for num, image_info in  enumerate(video_info):
                        ...
                        mask = cv2.imread(msk_path, 0) * 128

                        bboxes_pr = self.predict(image_source, mask)

                        ground_truth_path = os.path.join(ground_truth_dir_path, str(txt_num) + '.txt')
                        predict_result_path = os.path.join(predicted_dir_path, str(txt_num) + '.txt')
                        txt_num+=1
                                ...
                                if len(be_corrected_box_list) > 0 :
                                    for box_str in be_corrected_box_list:
                                        if len(box_str) > len('[None]'):
                                            local_str = box_str.split('[')[1].split(']')[0]
                                            bbox_mess = ' '.join(['be_corrected', local_str]) + '\n'
                                            f.write(bbox_mess)
                                if len(to_add_box)>len('[None]'):
                                    local_str = to_add_box.split('[')[1].split(']')[0]
                                    bbox_mess = ' '.join(['to_add', local_str ]) + '\n'
                                    f.write(bbox_mess)
                                if len(to_del_box)>len('[None]'):
                                    local_str  = to_del_box.split('[')[1].split(']')[0]
                                    bbox_mess = ' '.join(['to_del', local_str ]) + '\n'
                                    f.write(bbox_mess)
                        except:continue
                        ...
                        if True:
                            ...
                            save_video_folder = '/home/wangning/Desktop/data/yizhuang/experences'
                            if not os.path.exists(save_video_folder):
                                os.mkdir(save_video_folder)
                            save_folder = os.path.join(save_video_folder,task_id)
                            video_folder = os.path.join(save_folder,video_id)
                            if not os.path.exists(save_folder):
                                os.mkdir(save_folder)
                            if not os.path.exists(video_folder):
                                os.mkdir(video_folder)

                            image_result_path = os.path.join(video_folder, str(num))
                            cv2.imwrite(image_result_path +'src'+ '.jpg', image_source)
                            cv2.imwrite(image_result_path + 'msk' + '.png', mask)
                            image = utils.draw_bbox(image_source, bboxes_pr, show_label=self.show_label)
                            # cv2.imshow('image', image)

                            cv2.imwrite(image_result_path+'mask'+'.jpg', image)
    #
    #                     pass
    #
    #
